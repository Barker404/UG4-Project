...
def draw_graph(g, pos, round_no, messages, watched):
    # Draw this round's sharing
    seen_colours = []
    for node_index in g.nodes_iter():
        if watched is None or any(
                (messages[msgId] in g.node[node_index]['seen'][round_no])
    ...
    for edge in g.edges():
        if watched is None or any(
                (messages[msgId] in g.node[edge[0]]['shared'][round_no - 1]) or
                (messages[msgId] in g.node[edge[1]]['shared'][round_no - 1])
                for msgId in watched):
