...
I would like to thank my supervisor Rik Sarkar for proposing and assisting with this project. I would also like to thank Matthew Summers, Scott Postlethwaite, and Craig Snowden for proofreading my work, correcting my many grammatical and formatting errors.
...
\end{subfigure}
\caption{Delivery rates for the various display algorithms on chain graphs, as message count increases. Note that the y-axis shows values of 0-10\%. Aside from Algorithm 1, algorithm choice and message count have no effect.}
...
\caption{Algorithm 5 - Fractional Distance Priority ($f=0.5$)}
\end{subfigure}
\caption{Delivery rates for the various display algorithms on tree graphs, as message count increases. Note that the y-axis shows values of 0-40\%. Aside for Algorithm 1 which performs very poorly, algorithm choice has no effect.}
...
\caption{Messages delivered on grid graphs as the algorithm parameters are varied, for different message counts. The behaviour for grid graphs is similar to that of Kleinberg's graphs. The optimal values were found to be $p_{further}=0.5$ and $f=0.2$}
